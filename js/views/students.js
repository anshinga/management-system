...
export function renderStudents(state, filters = {}) {
  ...
  return `<div class="page-head students-page-head"><div class="students-page-title"><p class="eyebrow">名冊管理</p><h2>學生</h2><p>共 ${state.students.length} 位學生，變更會即時同步。</p></div><button class="button-primary" data-action="toggle-student-form">新增學生</button></div>
    <div class="toolbar students-toolbar"><div class="toolbar-start"><input class="input" id="student-search" value="${escapeAttribute(search)}" placeholder="搜尋姓名" /><select class="select" id="grade-filter"><option value="all">全部年級</option>${grades.map((item) => `<option value="${item}" ${String(item) === grade ? "selected" : ""}>${item} 年級</option>`).join("")}</select><select class="select" id="student-sort" aria-label="學生排序方式"><option value="grade" ${sort === "grade" ? "selected" : ""}>依年級排序</option><option value="lessons-desc" ${sort === "lessons-desc" ? "selected" : ""}>堂數：多到少</option><option value="lessons-asc" ${sort === "lessons-asc" ? "selected" : ""}>堂數：少到多</option></select></div></div>
    <div class="panel students-table-panel"><div class="schedule-wrap"><table class="data-table students-table"><thead><tr><th>學生</th><th>年級</th><th>堂數</th><th>期數</th><th>狀態</th><th>操作</th></tr></thead><tbody>${students.length ? students.map(renderRow).join("") : '<tr><td colspan="6" class="empty">找不到符合條件的學生。</td></tr>'}</tbody></table></div></div>`;
}

function renderRow(student) {
  const noteSummary = summarizeNote(student.note);
  return `<tr><td><strong>${escapeHtml(student.name)}</strong>${student.paymentPending ? ` <span class="pending-badge">${student.pendingPaymentCount} 期・待寄收費單</span>` : ""}${noteSummary ? `<div class="student-note-summary" title="${escapeAttribute(student.note)}">${escapeHtml(noteSummary)}</div>` : ""}</td><td><span class="student-desktop-value">${student.grade} 年級</span><span class="student-mobile-value">${student.grade}</span></td><td><span class="student-desktop-value">${student.currentLessonCount} / 24</span><span class="student-mobile-value">${student.currentLessonCount}</span></td><td><span class="student-desktop-value">第 ${student.currentTerm} 期</span><span class="student-mobile-value">${student.currentTerm}</span></td><td><span class="status-badge ${student.status}">${student.status === "active" ? "在讀" : "停課"}</span></td><td><button class="button-secondary student-edit-button" data-action="edit-student" data-student-id="${escapeAttribute(student.id)}">編輯</button></td></tr>`;
}
