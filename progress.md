...
- 沿用 `billingCycles.status` 的 `pending`／`paid`、`paidAt` 與學生既有提醒欄位及交易；新 `paidAt` 表示確認已寄送的時間，舊紀錄保留付款意義且不重新提醒。兩者不能只靠欄位辨別，未改寫歷史資料或新增欄位。
- 同步更新收費頁、導覽、今日點名、學生列表及相關錯誤提示與文件；保留確認後不可自行取消、跨期未處理提醒及第 20 堂撤銷的既有行為。
- 不修改 Firebase Rules、Indexes 或 Functions，不需要 Firebase 部署；介面與文件已更新，尚未提交或發布。
- 驗證：`npm.cmd test` 26 檔、179 項通過。實際 view、repository 與 CSS 搭配本機記憶體 Firestore 假資料，確認取消保留提醒、寄送後待寄筆數與點名紅字歸零、學生列表同步解除標籤，舊已結案紀錄未被改寫。桌面及 390px 手機排版正常，Console 無 error／warn。
- 原生 confirm 提示已確認包含收費單與具體期別；瀏覽器工具無法接續原生確認視窗，因此取消／確認回傳值由本機測試頁模擬，正式程式仍使用原生 confirm。未驗證正式登入後寫入，也未執行 Rules Emulator；本次資料欄位與交易邏輯均未更動。

## 2026-09-04 排課異常修復

- 使用者已要求將本次版本上線；以 `codex/fix-hidden-schedule-entries` 提交修復，經 GitHub 原有流程發布至 Pages。發布內容僅前端共用排課邏輯與測試／紀錄，不需要部署 Firebase Rules 或 Functions，也不執行正式資料遷移。實際發布結果以 GitHub PR 與 Pages 部署狀態為準。

- 正式唯讀查核已確認承諺案例：9/2、9/9 15:00 文件存在且為上學期固定排課，分別晚於對應例外約 19 秒、12 秒建立，卻仍被舊例外隱藏；9/16 同時段沒有例外所以正常。已排除該案例的文件缺漏及學期誤標，完成依建立／例外更新時間判斷的顯示修復，不改寫正式資料。
- 顯示與新增／單日移回共用例外有效性判斷：僅當排課 `createdAt` 明確晚於例外 `updatedAt` 時忽略舊例外；較晚的有效例外、同時或缺少時間戳記仍維持隱藏。自動沿用交易另重讀目標例外，避免查詢後新增的人工例外被自動沿用搶先覆蓋。9/2 16:30 的既有臨時排課保留。
- 驗證：`npm.cmd test` 共 26 檔、179 項通過（相較原始版本新增 20 項回歸測試），涵蓋正式案例時間順序、匯出一致性、較晚單日調課、重複加入、臨時排課衝突及自動沿用與人工例外競爭。
- 使用登入後唯讀取得的同一份正式資料，比較 `727a57b` 與本機修正後的分組程式：9/2、9/9 從不顯示恢復顯示，9/16 維持顯示；瀏覽器 Console 無 error／warn。正式資料庫沒有新增、刪除或更新。

- 分支：`codex/fix-hidden-schedule-entries`，針對本週排課隱藏但新增判斷為已存在的問題。
- 已找到顯示端套用 `scheduleOverrides`，新增／一般拖曳未處理目標例外的判斷落差；先補上 9/5 週六與 8/31 跨學期週的本機回歸案例。
- 修正前重現 6 項失敗。新增與一般拖曳改為在交易中讀取目標例外，明確重新加入固定排課時解除該時段例外；保留臨時排課不沿用的規則。若同日同時段文件屬於其他學期，明確拒絕並提示核對，不覆寫資料。
- 另以實際排課 view、repository 與 CSS 搭配本機記憶體假資料，在瀏覽器操作 9/5 週六「新增學生」恢復隱藏排課，再切換 9/12 確認正常沿用；Console 無 error／warn。
- 自我審查：已檢查實際 diff 與修改範圍。未新增依賴或修改 Firestore Rules；本機找不到 Java，未執行 Rules Emulator，亦未驗證正式資料寫入。
- 週六其他學生尚未逐一查核，不可宣稱所有缺課已確認為相同原因；不自動回填或修改正式資料。使用者確認目前未使用選課活動，皆手動排課。
- 唯讀查核頁位於 Codex 本機產物目錄，只提供指定學生查核及修正前後比較，不納入網站發布；頁面明示正式網站尚未更新。

> 最後更新：2026-09-04
> 更新時的分支：`codex/fix-hidden-schedule-entries`
> 修復基底 Git HEAD：`727a57b`；下方原有檢查點保留 2026-08-21 的交接背景，以本文件頂端最新紀錄為準。
